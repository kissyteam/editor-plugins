...
*/
KISSY.add('editor-plugins/lib/fake-objects', [
    'editor',
    'html-parser',
    'util',
    'node',
    ...
    Editor.addMembers({
        ...
        createFakeElement: function (realElement, className, realElementType, isResizable, outerHTML, attrs) {
            ...
            var self = this,
                ...
                existClass = util.trim(realElement.attr('class')), attributes = {
    ...
    var htmlFilterRules = {
            tags: {
                ...
                $: function (element) {
                    ...
                    var realElement = realFragment && realFragment.childNodes[0];    // If we have width/height in the element, we must move it into
                                                                                     // the real element.
                    ...
                    if (realElement) {
                        ...
                        if (style) {
                            ...
                            var match = /(?:^|\s)width\s*:\s*(\d+)/i.exec(style), width = match && match[1];    // Get the height from the style.
