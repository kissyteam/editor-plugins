...
*/
KISSY.add('editor-plugins/lib/remove-format/cmd', [
    'editor',
    'dom'
                ...
                editor.addCommand('removeFormat', {
                    exec: function () {
                        ...
                        for (var i = 0, range; range = ranges[i]; i++) {
                            ...
                            range.enlarge(KER.ENLARGE_ELEMENT);    // Bookmark the range so we can re-select it after processing.
                            // Bookmark the range so we can re-select it after processing.
                            var bookmark = range.createBookmark(),
                                // The style will be applied within the bookmark boundaries.
                                startNode = bookmark.startNode, endNode = bookmark.endNode;    // We need to check the selection boundaries (bookmark spans) to break
                            ...
                            breakParent(endNode);    // Navigate through all nodes between the bookmarks.
                            // Navigate through all nodes between the bookmarks.
                            var currentNode = startNode    // start from sibling , because obvious bookmark has no children
._4eNextSourceNode(true, Dom.NodeType.ELEMENT_NODE, undefined, undefined);
                            while (currentNode) {
                                ...
                                // Cache the next node to be processed. Do it now, because
                                // currentNode may be removed.
                                var nextNode = currentNode._4eNextSourceNode(false, Dom.NodeType.ELEMENT_NODE, undefined, undefined);    // This node must not be a fake element.
                                // This node must not be a fake element.
                                if (!(currentNode.nodeName() === 'img' && (currentNode.attr('_ke_real_element') || // 占位符
