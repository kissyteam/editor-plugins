...
*/
KISSY.add('editor-plugins/lib/word-filter', [
    'html-parser',
    'util',
    'node',
    'ua'
 ...
 */
    ...
    var $ = require('node'), UA = require('ua'), dtd = HtmlParser.DTD, wordFilter = new HtmlParser.Filter(), cssLengthRelativeUnit = /^([.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz){1}?/i,
        ...
        ], alphabets = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';    // Convert roman numbering back to decimal.
 ...
 */
    ...
    function addStyle(elem, name, value, isPrepend) {
        var styleText, addingStyleText = '', style;    // name/value pair.
        // name/value pair.
    ...
    var filters = {
            ...
            flattenList: function (element, level) {
                ...
                var listStyleType;    // All list items are of the same type.
                ...
                case '1':
                    ...
                    break;    // TODO: Support more list style type from MS-Word.
                }
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    if (child.nodeName in dtd.$listItem) {
                        var listItemChildren = child.childNodes || [], count = listItemChildren.length, last = listItemChildren[count - 1];    // Move out nested list.
                        // Move out nested list.
                        if (last.nodeName in dtd.$list) {
                            element.insertAfter(child);    // Remove the parent list item if it's just a holder.
                            // Remove the parent list item if it's just a holder.
                            if (!--listItemChildren.length) {
                                element.removeChild(children[i--]);
                            }
                        }
                        child.setTagName('ke:li');    // Inherit numbering from list root on the first list item.
                ...
                element.nodeName = element.tagName = null;    // We're loosing tag name here, signalize this element as a list.
     ...
     */
            assembleList: function (element) {
                var children = element.childNodes || [], child, listItem,
                    ...
                    lastIndent, lastListItem,
                    // The previous one just been added to the list.
                    list,
                    // Current staging list and it's parent list if any.
                    openedLists = [], previousListStyleType, previousListType;    // Properties of the list item are to be resolved from the list bullet.
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    if ('ke:li' === child.nodeName) {
                        ...
                        // been resolved from a pseudo list item's margin value, even get
                        // no indentation at all.
                        listItemIndent = Number(listItem.getAttribute('ke:indent'));    // We're moving out of the current list, cleaning up.
                        ...
                        var styleStr = filters.stylesFilter([
                                ...
                                [
                                    ...
                                    function (margin) {
                                        ...
                                        margin = convertToPx(values[3] || values[1] || values[0]);    // Figure out the indent unit by checking the first time of incrementation.
    ...
    (function () {
        ...
        wordFilter.addRules({
            tagNames: [// Remove script, meta and link elements.
                [
                    /meta|link|script/,
                    ''
                ]],
            ...
            tags: {
                ...
                $: function (element) {
                    var tagName = element.nodeName || '';    // Convert length unit of width/height on blocks to
                    ...
                    if (tagName.match(/h\d/)) {
                        element.filterChildren();    // Is the heading actually a list item?
                        // Is the heading actually a list item?
                        if (resolveListItem(element)) {
                            return;
                        }
                    }    // Remove inline elements which contain only empty spaces.
                    else // Remove inline elements which contain only empty spaces.
                    ...
                    else // Remove element with ms-office namespace,
                    // with it's content preserved, e.g. 'o:p'.
                    if (tagName.indexOf(':') !== -1 && tagName.indexOf('ke') === -1) {
                        element.filterChildren();    // Restore image real link from vml.
                ...
                'style': function (element) {
                    if (UA.gecko) {
                        ...
                        var styleDefSection = onlyChild(element).nodeValue.match(/\/\* Style Definitions \*\/([\s\S]*?)\/\*/), styleDefText = styleDefSection && styleDefSection[1], rules = {};    // Storing the parsed result.
                        // Storing the parsed result.
                        if (styleDefText) {
                            styleDefText    // Remove line-breaks.
.replace(/[\n\r]/g, '')    // Extract selectors and style properties.
.replace(/(.+?)\{(.+?)\}/g, function (rule, selectors, styleBlock) {
                                ...
                                for (var i = 0; i < length; i++) {
                                    ...
                                    util.trim(selectors[i]).replace(/^(\w+)(\.[\w-]+)?$/g, function (match, tagName, className) {
                                        tagName = tagName || '*';
                                        className = className.substring(1, className.length);    // Reject MS-Word Normal styles.
                ...
                'p': function (element) {
                    ...
                    element.filterChildren();    // Is the paragraph actually a list item?
                ...
                'font': function (element) {
                    ...
                    if ('font' === parent.name)
                        ...
                        // Convert the merged into a span with all attributes preserved.
                        {
                            styleText = styleText || '';    // IE's having those deprecated attributes, normalize them.
                            ...
                            var size = element.getAttribute('size');    // TODO: Mapping size in ranges of xx-small,
                ...
                'span': function (element) {
                    ...
                    if (isListBulletIndicator(element)) {
                        ...
                        if (listType) {
                            var marker = createListBulletMarker(listType, listSymbol);    // Some non-existed list items might be carried by an inconsequential list,
                    ...
                    // Update the src attribute of image element with href.
                    var styleText = element.getAttribute('style');    // Assume MS-Word mostly carry font related styles on <span>,
                    ...
                    [
                        ...
                        function (value, element, name) {
                            if (element.nodeName in {
                                ...
                                var indentStyleName = 'margin-left';    // Extract component value from 'margin' shorthand.
            ...
            comment: UA.ie ? function (value, node) {
                var imageInfo = value.match(/<img.*?>/), listInfo = value.match(/^\[if !supportLists\]([\s\S]*?)\[endif\]$/);    // Seek for list bullet indicator.
                ...
                if (UA.gecko && imageInfo) {
                    var img = new HtmlParser.Parser(imageInfo[0]).parse().childNodes[0], previousComment = node.previousSibling,
                        // Try to dig the real image link from vml markup from previous comment text.
                        imgSrcInfo = previousComment && previousComment.toHtml().match(/<v:imagedata[^>]*o:href=[''](.*?)['']/), imgSrc = imgSrcInfo && imgSrcInfo[1];    // Is there a real 'src' url to be used?
