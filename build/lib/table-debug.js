...
*/
KISSY.add('editor-plugins/lib/table', [
    'editor',
    './dialog-loader',
    './contextmenu',
    './button',
 ...
 */
    ...
    function getSelectedCells(selection) {
        ...
        Editor.Utils.clearAllMarkers(database);    // Restore selection position.
    ...
    function clearRow($tr) {
        ...
        var $cells = $tr.cells;    // Empty all cells.
        // Empty all cells.
    ...
    function insertRow(selection, insertBefore) {
        ...
        // Create a clone of the row.
        var newRow = row.clone(true);    // Insert the new row before of it.
        // Insert the new row before of it.
        newRow.insertBefore(row);    // Clean one of the rows to produce the illusion of
    ...
    function deleteRows(selectionOrRow) {
        ...
        if (selectionOrRow instanceof Editor.Selection) {
            var cells = getSelectedCells(selectionOrRow), cellsCount = cells.length, rowsToDelete = [], cursorPosition, previousRowIndex, row, nextRowIndex;    // Queue up the rows - it's possible and
            ...
            var rows = table[0].rows, rowCount = rows.length;    // Where to put the cursor after rows been deleted?
    ...
    function insertColumn(selection, insertBefore) {
        ...
        // Get the cell's table.
        var table = cell.parent('table'), cellIndex = cell[0].cellIndex;    // Loop through all rows available in the table.
        // Loop through all rows available in the table.
        for (var i = 0; i < table[0].rows.length; i++) {
            var $row = table[0].rows[i];    // If the row doesn't have enough cells, ignore it.
    ...
    function getFocusElementAfterDelCols(cells) {
        var cellIndexList = [], table = cells[0] && cells[0].parent('table'), i, length, targetIndex, targetCell;    // get the cellIndex list of delete cells
    ...
    // table 编辑模式下显示虚线边框便于编辑
    var showBorderClassName = 'ke_show_border', cssTemplate = // IE6 don't have child selector support,
        ...
        (UA.ie === 6 ? [
            ...
            'border : #d3d3d3 1px dotted',
            '}'
        ]).join(''), cssStyleText = cssTemplate.replace(/%2/g, showBorderClassName), extraDataFilter = {
            tags: {
                'table': function (element) {
                    var cssClass = element.getAttribute('class'), border = parseInt(element.getAttribute('border'), 10);
                    if (!border || border <= 0) {
                        element.setAttribute('class', util.trim((cssClass || '') + ' ' + showBorderClassName));
                    }
                }
            }
        }, extraHTMLFilter = {
        ...
        pluginRenderUI: function (editor) {
            ...
            var self = this, handlers = {
                    ...
                    '\u5220\u9664\u8868\u683C': function () {
                        ...
                        editor.execCommand('save');    // Maintain the selection point at where the table was deleted.
                        // Maintain the selection point at where the table was deleted.
                        selection.selectElement(table);
                        var range = selection.getRanges()[0];
                        range.collapse();
                        selection.selectRanges([range]);    // If the table's parent has only one child,
