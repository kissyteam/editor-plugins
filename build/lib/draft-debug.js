...
*/
KISSY.add('editor-plugins/lib/draft', [
    'editor',
    'json',
    'event/dom',
    './local-storage',
 ...
 */
    ...
    util.augment(Draft, {
        ...
        _getDrafts: function () {
            ...
            if (!self.drafts) {
                ...
                if (str) {
                    ...
                    drafts = localStorage === window.localStorage ? Json.parse(util.urlDecode(str)) : str;
                }
                self.drafts = drafts;
            }
            return self.drafts;
        ...
        save: function (auto) {
            var self = this, drafts = self._getDrafts(), editor = self.editor,
                //不使用rawdata
                //undo 只需获得可视区域内代码
                //可视区域内代码！= 最终代码
                //代码模式也要支持草稿功能
                //统一获得最终代码
                data = editor.getFormatData();    //如果当前内容为空，不保存版本
            //如果当前内容为空，不保存版本
            if (!data) {
                return;
            }
