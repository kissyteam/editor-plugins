...
*/
KISSY.add('editor-plugins/lib/contextmenu', [
    'editor',
    'menu',
    './focus-fix',
    'event/dom',
 ...
 */
    ...
    Editor.prototype.addContextMenu = function (id, filter, cfg) {
        ...
        self.docReady(function () {
            var doc = self.get('document');    // 编辑器获得焦点，不会触发 menu el blur？
        ...
        function showNow(ev) {
            var t = $(ev.target);    // ie 右键作用中，不会发生焦点转移，光标移动
            ...
            setTimeout(function () {
                ...
                menu.show();
                window.focus();
                document.body.focus();    // 防止焦点一直在 el，focus 无效
                // 防止焦点一直在 el，focus 无效
                menu.focus();
            }, 30);
        }
