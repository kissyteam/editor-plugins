...
*/
KISSY.add('editor-plugins/lib/dent-cmd', [
    'editor',
    './list-utils',
    'dom',
    'node',
 ...
 */
    ...
    function indentList(range, listNode, type) {
        ...
        var indentOffset = type === 'indent' ? 1 : -1, startItem = itemsToMove[0], lastItem = itemsToMove[itemsToMove.length - 1], database = {};    // Convert the list Dom tree into a one dimensional array.
        // Convert the list Dom tree into a one dimensional array.
        var listArray = ListUtils.listToArray(listNode, database);    // Apply indenting or outdenting on the array.
        ...
        // Convert the array back to a Dom forest (yes we might have a few subtrees now).
        // And replace the old list with the new forest.
        var newList = ListUtils.arrayToList(listArray, database, null, 'p');    // Avoid nested <li> after outdent even they're visually same,
        ...
        if (pendingList && pendingList.length) {
            for (i = 0; i < pendingList.length; i++) {
                var li = pendingList[i], followingList = li;    // Nest preceding <ul>/<ol> inside current <li> if any.
    ...
    function indentBlock(range, type) {
        var iterator = range.createIterator(), block;    //  enterMode = 'p';
        //  enterMode = 'p';
    ...
    function indentEditor(editor, type) {
        ...
        var walker;    // Avoid selection anchors under list root.
        ...
        if (nearestListBlock) {
            ...
            var rangeStart = range.startContainer, indentWholeList = firstListItem[0] === rangeStart[0] || firstListItem.contains(rangeStart);    // Indent the entire list if  cursor is inside the first list item. (#3893)
