...
*/
KISSY.add('editor-plugins/lib/undo/cmd', [
    'editor',
    'ua',
    'util'
 ...
 */
    function Snapshot(editor) {
        ...
        //内容html
        self.contents = contents;    //选择区域书签标志
        //选择区域书签标志
        self.bookmarks = selection && selection.createBookmarks2(true);
    }
    Snapshot.prototype = {
        equals: function (otherImage) {
            var self = this, thisContents = self.contents, otherContents = otherImage.contents;    // 不比较书签！
 ...
 */
     ...
     */
        ...
        //当前所处状态对应的历史栈内下标
        self.index = -1;
        self.editor = editor;    //键盘输入做延迟处理
        //键盘输入做延迟处理
        self.bufferRunner = util.buffer(self.save, 500, self);
        self._init();
    }
    var
        //editingKeyCodes = { /*Backspace*/ 8:1, /*Delete*/ 46:1 },
        modifierKeyCodes = {
            /*Shift*/
            16: 1,
            /*Ctrl*/
     ...
     */
        save: function (buffer) {
            ...
            var self = this, history = self.history, l = history.length, index = self.index;    //前面的历史抛弃
            //前面的历史抛弃
