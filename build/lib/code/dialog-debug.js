...
*/
KISSY.add('editor-plugins/lib/code/dialog', [
    'util',
    'editor',
    'menubutton',
    'dom',
 ...
 */
    ...
    var codeTypes = [
        ...
        ], bodyTpl = '<div class="{prefixCls}code-wrap">' + '<table class="{prefixCls}code-table">' + '<tr>' + '<td class="{prefixCls}code-label">' + '<label for="ks-editor-code-type">' + '\u7C7B\u578B\uFF1A' + '</label>' + '</td>' + '<td class="{prefixCls}code-content">' + '<select ' + 'id="ks-editor-code-type" ' + ' class="{prefixCls}code-type">' + util.map(codeTypes, function (codeType) {
    ...
    util.augment(CodeDialog, {
        initDialog: function () {
            ...
            d = self.dialog = new Dialog4E({
                ...
                mask: true,
                headerContent: '\u63D2\u5165\u4EE3\u7801',
                bodyContent: util.substitute(bodyTpl, { prefixCls: prefixCls }),
                footerContent: util.substitute(footTpl, { prefixCls: prefixCls })
            }).render();
            el = d.get('el');
